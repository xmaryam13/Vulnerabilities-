#imported the necessary modules and libraries
...
#create the login function for the login page
@views.route('/')
@cross_origin()
def login():
    try:
...
@cross_origin()
#create the function for the dashboard which displayes all the information on the dashboard 
def dashboard():
    try:
...
@views.route('/profile')
@cross_origin()
#create different profiles for different users selecting them from different columns
def profile():
    try:
...
@views.route('/order')
@cross_origin()
#create the order function that gets information on what the user has purchased 
def order():
    try:
...
#creates the help page function for the user when they want to send a message for help. This does not restrict users to
#enter a form of image type which is how hackers are able to attack people's computers by installing a virus
def help_page():
    try:
